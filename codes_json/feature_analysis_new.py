...
def evaluate(X_tests, y_test, index_test, clf, name, sent_print=True):
    ...
    y_hats = []
    print(len(clf), len(X_tests))
    for i,clfi in enumerate(clf):
        print(i)
        y_hats.append(clfi.predict_proba(X_tests[i]))
...
def main(working_dir, args):
    ...
    for name, clf in zip(sel_names, sel_classifiers):
        ...
        try:
            ...
            with open(working_dir + "/comb" + name + '.pkl', 'rb') as f1:
                clf1, clf2 = pickle.load(f1)
            # with open(working_dir + "/" + name + '.pkl', 'rb') as f1:
            #     clf2 = pickle.load(f1)
        except:
            estimator = SVC
            kargs = {"C" : 1, "kernel" : "linear", "probability" : True}
            clf1 = EnsembleEstimator(estimator, kargs)
            clf2 = EnsembleEstimator(estimator, kargs)
